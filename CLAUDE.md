...
- **Codex review-bot comments are a PRE-MERGE gate on every PR (owner rule 2026-07-29, severity rule 2026-07-30):** after pushing a PR, wait for and check the Codex bot's review comments (`chatgpt-codex-connector`), triage them yourself, and reply on the PR with what was fixed/why anything was deferred — WITHOUT waiting to be asked. **P1 findings block the merge and are fixed on the same branch immediately. P2 findings do NOT block a release the owner is waiting on** — log them on the PR as deferred, resolve the threads, merge, and ship the fixes in the next batch (the bot finds new P2 edges every round on fresh code; the P1/P2 split is what terminates the loop). If the bot comments only after a merge already happened, ship P1 fixes as an immediate follow-up PR.
