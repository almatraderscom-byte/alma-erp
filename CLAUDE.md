...
- **Router-worker (multi-model):** the head delegates discrete sub-tasks to specialist sub-agents via the tier router (`src/agent/lib/models/`: `registry.ts`, `tier-router.ts`, `routing-config.ts`, `subagent.ts`, `specialist-roles.ts`, `adapters/`). The CRITICAL tier — **finance / data-analysis only** (the `analyst` role) — is **hard-guarded to Claude only** (`assertCriticalTierUsesClaude` + `CRITICAL_SPECIALIST_ROLES`), because money decisions must not run on a cheap model. Everything else (staff/`ops`, `cs`, orders coordination, research, marketing, content) is **NON-critical and intentionally runs on DeepSeek** (`or-deepseek-v4-flash`) to cut cost — this is a deliberate owner decision, not a regression. HEAVY / LIGHT tiers run cheaper models (OpenRouter DeepSeek / Gemini; Qwen optional). OpenRouter failures fall back to native Gemini → Claude. Customer-facing Bangla quality stays gated via `bangla-output-gate.ts`; the head (owner-facing) stays on Claude.
