...
# next-env.d.ts is regenerated by every `next build`, so the PREVIOUS engine
# build re-dirties it and would block every later deploy — reset that one
# known build artifact before the check (it is generated output, never
# reviewed-code drift; live hit 2026-08-26).
