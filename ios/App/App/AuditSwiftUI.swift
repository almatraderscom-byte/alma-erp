...
struct AuditScreen: View {
    ...
    // ── KPI strip (Total / OK / FAIL over the loaded window) — bento dark hero
    //    (owner spec 2026-07-08): same three counts, presentation only. ──

    private var kpiStrip: some View {
        AudBentoHeroCard(total: vm.total, ok: vm.okCount, fail: vm.failCount)
    }
...
// MARK: - Bento components (Audit-owned copies of the Dashboard board language —
...
@available(iOS 17.0, *)
private func audMotionOK(_ reduceMotion: Bool) -> Bool {
...
@available(iOS 17.0, *)
private struct AudCountUp: View {
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    let target: Int
    ...
    var body: some View {
        ...
        AudCountUpText(value: shown)
            .animation(audMotionOK(reduceMotion) ? .spring(duration: 0.9, bounce: 0) : nil,
                       value: shown)
            .onAppear {
                guard !appeared else { return }
                if audMotionOK(reduceMotion) {
                    appeared = true
                } else {
                    var tx = Transaction(); tx.disablesAnimations = true
...
@available(iOS 17.0, *)
private struct AudCountUpText: View, Animatable {
    var value: Double
...
/// deep indigo base + violet/coral washes + a sage hint). Window total count-up plus
/// the OK / FAIL split — the same three counts the old strip showed.
@available(iOS 17.0, *)
private struct AudBentoHeroCard: View {
    let total: Int
    let ok: Int
    let fail: Int
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            Text("অডিট লগ · TOTAL").font(.system(size: 10, weight: .bold)).tracking(0.8)
                .foregroundStyle(AuditPalette.goldLt)
            AudCountUp(target: total)
                .font(.system(size: 40, weight: .heavy)).monospacedDigit()
                .foregroundStyle(.white)
                .lineLimit(1).minimumScaleFactor(0.6)
                .padding(.top, 8)
            Text(fail == 0 ? "সব চেক পাশ — ফেইল নেই" : "\(fail)টি ফেইল দেখা দরকার")
                .font(.caption2).foregroundStyle(.white.opacity(0.6)).padding(.top, 5)

            HStack(alignment: .top, spacing: 0) {
                heroStat(label: "OK", value: ok,
                         tint: AuditPalette.green400, sub: "সফল")
                Rectangle().fill(.white.opacity(0.14)).frame(width: 1)
                    .padding(.vertical, 2).padding(.horizontal, 14)
                heroStat(label: "Fail", value: fail,
                         tint: fail > 0 ? AuditPalette.red500 : .white, sub: "ব্যর্থ")
                Spacer(minLength: 0)
            }
            .padding(.top, 14)
        }
