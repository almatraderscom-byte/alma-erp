# SPEC-120 Architecture Scan — Autonomy and approval adversarial tests

## Forbidden-dependency / bypass scan (one-way rule: ERP must not import agent)

```text
$ rg agent/autonomy|agent/approvals in ERP
...
`scripts/architecture`, `src/agent/contracts`) plus `artifacts/SPEC-120`.
