# SPEC-175 Architecture Scan — Finance and invoice workflow templates

## Forbidden-dependency / bypass scan (one-way rule: ERP must not import agent)

```text
$ rg agent/specialists|agent/workflow-templates in ERP
...
`scripts/architecture`, `src/agent/contracts`) plus `artifacts/SPEC-175`.
