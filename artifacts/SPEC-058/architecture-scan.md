# SPEC-058 Architecture Scan — Memory privacy and tenant isolation

## Forbidden-dependency / bypass scan (one-way rule: ERP must not import agent)

```text
$ rg agent/memory in ERP
...
```text
# memory must be deterministic — no provider/model/db CALL (embeddings passed in):
  NONE — deterministic; embeddings are inputs (INV-01)
```

## Ownership-zone diff check

All changes are confined to owned zones (`docs/architecture`,
`scripts/architecture`, `src/agent/contracts`) plus `artifacts/SPEC-058`.
