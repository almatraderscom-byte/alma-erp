# SPEC-199 Architecture Scan — Workflow optimization recommendations

## Forbidden-dependency / bypass scan (one-way rule: ERP must not import agent)

```text
$ rg agent/observability|agent/release in ERP
...
`scripts/architecture`, `src/agent/contracts`) plus `artifacts/SPEC-199`.
