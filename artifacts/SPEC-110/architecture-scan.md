# SPEC-110 Architecture Scan — Authorization bypass CI and runtime gate

## Forbidden-dependency / bypass scan (one-way rule: ERP must not import agent)

```text
$ rg agent/policy|agent/identity in ERP
...
`scripts/architecture`, `src/agent/contracts`) plus `artifacts/SPEC-110`.
