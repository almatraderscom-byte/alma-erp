# SPEC-140 Architecture Scan — Workflow durability chaos tests

## Forbidden-dependency / bypass scan (one-way rule: ERP must not import agent)

```text
$ rg agent/workflows|worker/workflows in ERP
...
`scripts/architecture`, `src/agent/contracts`) plus `artifacts/SPEC-140`.
