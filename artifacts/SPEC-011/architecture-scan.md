# SPEC-011 Architecture Scan — Single admission gateway
...
`scripts/architecture`, `src/agent/contracts`) plus `artifacts/SPEC-011`.
See `changed-files.md`.

## Executable gate

```text
$ node scripts/architecture/check-forbidden-imports.mjs (G01 gate — ERP must not import agent, incl control-plane)
forbidden-import scan: 1994 files scanned
