...
export async function runStreamedTurn({ supabase, job, redisUrl, telegramBot }) {
  const { turnId, conversationId, message, files, projectId, personalMode } = job.data ?? {}
  ...
  try {
    const res = await fetch(`${getAppUrl()}/api/assistant/chat?stream=true`, {
      ...
      body: JSON.stringify({ conversationId, message, files, projectId, personalMode, turnId }),
