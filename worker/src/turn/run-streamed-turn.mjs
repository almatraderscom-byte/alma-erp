...
export async function runStreamedTurn({ supabase, job, redisUrl, telegramBot }) {
  const { turnId, conversationId, message, files, projectId, personalMode, clientRequestId, internalControl } = job.data ?? {}
  ...
  try {
    const res = await fetch(`${getAppUrl()}/api/assistant/chat?stream=true`, {
      ...
      body: JSON.stringify({ conversationId, message, files, projectId, personalMode, turnId, clientRequestId, internalControl }),
