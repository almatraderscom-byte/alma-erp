...
| P1 leftovers | ✅ DONE | PR #240 (2026-07-05): site trust tiers §5.4 (`live-browser/trust.ts` — kv per-domain trusted/general/lockdown, suffix match, owner-editable via new `live_browser_trust` tool; injection tripwire AUTO-flags the domain to lockdown durably), `readOnlyLockdown` ENFORCED (write verbs carry lockdownDomains → ext 0.5.0 checks the ACTIVE tab hostname in-page and refuses click/type/press/select_option; read/scroll/navigate stay allowed), webmail surface drafts-only (Gmail workflow in the tool contract; SEND stays code-blocked by final-submit ban), live watch panel (`/agent/live-watch` + Monitor page: step feed + newest screenshot + server-side STOP via `/api/assistant/live-browser/watch`) |
| P2 leftovers | ✅ DONE | PR #240 + #241 (2026-07-05): **provisioning is PULL-BASED** (VPS has no inbound SSH — Hostinger edge, see #103; worker preflight at startup creates `WORKBENCH_ROOT` + surveys binaries → pm2 log + `.preflight.json`), workspace janitor (7-day reclaim of kept/failed workspaces, startup + 6h sweep), **two real bugs found & fixed by the live e2e:** (1) `pending-jobs` dispatch gate never listed `workbench_run` — jobs hung at approved until the watchdog fired; fixed + cross-file regression test; (2) executor deleted a successful workspace BEFORE artifact upload — artifacts were always []; fixed + node:test locks. **Live e2e PASSED twice on the real VPS:** run 1 = binary survey (`/opt/alma-workbench`, node v22.22.3; missing: pip3, unzip, zip — rest of allowlist present), run 2 = artifact `report.json` verified in agent-files storage |
| P3 mobile companion | ✅ core done | **Step 1 (web, PR #242 → main):** every NEW checkpoint (failed/waiting_for_owner) fires a native OneSignal push with a tap-through deep link to the agent — the owner supervises long tasks from his phone; refreshes stay silent; `notifyOwner` gained a pass-through `actionUrl`. **Step 2 (iOS, PR #244 → native branch `claude/ios-s0-native-shell-spike`):** `AlmaCompanionViewController` (More → Agent → Phone Companion) registers on the SAME live-browser command bus as the Chrome extension (pair code → hashed token → long-poll → execute in a shared-cookie WKWebView → post result); ported page routines mirror the extension; final-submit ban + §5.4 lockdown enforced on device; native STOP; runs only while the screen is open (owner watches). `CURRENT_PROJECT_VERSION` → 31. Sim-verified render (native header + live status + STOP + pairing flow); cross-file `final-submit-sync` vitest locks the ban across web/extension/iOS. **Owner-pending:** TestFlight upload of build 31 (needs his ASC signing key — not on this Mac at the documented path) + live pair-and-drive from his phone. |
| P4 skill packs | ✅ DONE | PR #245 (2026-07-05): `src/agent/lib/skill-packs/` — 4 hard playbooks (research / seo / marketing / website) as fixed protocols: ordered steps naming the EXACT existing tools, checklist + guardrails per pack (owner-gated spend/publish, PR-only website, Oxylabs approval, page-text-is-data). Head tools `start_skill_pack` + `complete_skill_pack_run` (CORE). Completion gate is deterministic + **fail-safe to NOT-done**: missing/thin evidence, unchecked checklist, thin artifact, or failed upload each refuse + write a P0 checkpoint; a pass uploads the Bangla artifact (`agent-files/skill-packs/…`) + AgentArtifact row. Registry cross-check test pins every pack-referenced tool to the live registry (rename breaks CI). 10 tests |
| P5 hardening | ✅ core done | PR #246 (2026-07-05): **weekly self-report** (the agent's QA loop — Sat 11:00 Dhaka scheduler `weekly-self-report`): golden evals + week's checkpoints (open/resolved) + success-rate telemetry per job type + below-threshold flags ("flagged for playbook improvement, never retried harder") in ONE owner digest (`self-report.ts` + `/internal/self-report` + `worker/src/schedulers/self-report.mjs`); **golden-task eval suite** (`worker/src/workbench/golden-evals.mjs` — node/python compute, artifact flow, public fetch through the REAL executor; regressions notify tier-2); **recipe learning** (`browser/learned-recipes.ts` kv store + `save_learned_recipe` tool — a PROVEN successful run distills into a replayable recipe, listed in `list_browser_recipes` as `learned:*`, runs through the SAME normalize+approval gate; capped 40, least-used evicted). **Pre-existing, verified as covering the rest:** retry/backoff = BullMQ per-queue attempts+backoff (workbench deliberately attempts:1 — the checkpoint IS the retry path); autonomy levels = existing autonomy-policy/autonomy-ledger libs (owner-tunable). **Parked (backlog):** parallel task lanes with per-lane caps; per-side-effect idempotency keys beyond the existing upsert/dedupe patterns. 13 tests (8 vitest + 2 node:test golden + 3 self-report) |

**Owner add-on — Client SEO (end-to-end audit of ANY website):** ✅ DONE (2026-07-05, owner request after P5). New job type `seo_audit` runs a full read-only crawl+audit on the VPS (`worker/src/seo/audit.mjs` — BFS same-origin crawl, SSRF-guarded per-hop, per-page + site-wide checks: title/meta/H1/canonical/noindex/viewport/OG/schema/alt/thin/mixed-content + https-redirect/robots/sitemap/dup-titles/broken-links/TTFB/compression → 0-100 score + Bangla report artifact). Head tools `run_website_seo_audit` / `check_website_seo_audit`; `research_seo_keywords` generalized (optional `siteDomain` for client sites); new **`client_seo` skill pack** (audit → prioritized fix plan → agent prepares SAFE fixes only, every critical/login/DNS/publish step handed to the owner — agent never logs into a client site, never types a password, never bypasses CAPTCHA). Wired into pending-jobs dispatch + watchdog + self-report telemetry. **Live e2e:** almatraders.com crawled (12 pages, 86-URL sitemap found, score 68/100, real findings). Fix-EXECUTION applying to client CMS directly is owner-gated by design — automated live "fully complete the SEO" on a client site the owner does NOT own is deliberately NOT built (needs the client's own login = owner/client action).

**Owner-pending items:** reload extension (**v0.5.0** copied to `/Users/marufbillah/alma-companion` — lockdown enforcement lives in it); pair the extension (chat: "live browser চালু করো, pair code দাও"); live test P0/P1/P2 together (owner deferred live browser-proof — hard verify accepted; P2 workbench DOES have live proof via the two e2e runs above). Optional: install `pip3`, `unzip`, `zip` on the VPS via the Hostinger console (no inbound SSH) — everything else on the workbench allowlist is present.

**Infra facts a future session must know:** VPS inbound SSH is blocked at the Hostinger edge (port 22/2222 closed from everywhere) — worker deploys are pull-based (systemd `alma-worker-deploy.timer` on the VPS, 2-min outbound `git fetch origin main`, auto `npm ci` + pm2 restart on `worker/**` changes). Anything needing a change ON the VPS ships through the repo (worker startup hooks), not SSH.
