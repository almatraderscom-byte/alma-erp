...
- Full family = two protected pairs + exactly ONE group composite (pair unit inserted at 1.0). **LIVE-PROVEN on production 2026-07-17 (update):** after the owner saved ALMA mother/ALMA daughter models and PR #422 routed chain VTON steps to Fal FASHN v1.6 (owner directive — direct FASHN was OutOfCredits), a full পুরো ফ্যামিলি protected run completed end to end ENTIRELY on Fal: 2 pair chains (adult fal try-on → child garment → child fal try-on → pair composite "2 জন যাচাই") + exactly one group composite — final frame "🛡 প্রোটেক্টেড কম্পোজিট · **4 জন যাচাই**" with all four in the matching set. Remaining quality caveat: source product photos are marketing composites with baked-in text that bleeds into shots — clean garment-only golden fixtures (owner action) fix this, not code.
...
- Honest limits (documented in code): people touching in the supplier photo merge into ONE component (olive photo) so the child piece isn't split — apart-standing sets (like the maroon মা-মেয়ে set) will split; only DARK plates are detected (bright plates risk lamp/sky false positives).

### Post-program hardening 2: no AI child garment + full overlay-text kill chain (2026-07-17, PRs #436–#442 — owner call "baccar jnne alada keno garment banano lagbe?")

- **PR #436 — AI child_garment step REMOVED from all chains** (owner design call, live-proven right): VTON engines size the garment to the target body, so the child try-on reuses the SAME adult garment reference (or the real supplier child crop when prep splits one). The plastic/cartoon child shots came from the AI-imagined garment; with the real garment the child renders photorealistic. Chains are now 4 steps; legacy child-garment kv cache is no longer read.
- **PRs #437–#442 — garment-prep v3 overlay-text kill chain** (kv `garment_prep_v3:`, crops `-v3pN`): (a) crop alpha limited to its OWN person component — disconnected overlay graphics vanish free; (b) tightened bright-glyph smear fill (prayer-cap false positive fixed by the thin-line rule); (c) residual coloured/smoke-connected lettering boxed by a narrow mechanical Gemini call and repainted by FLUX Fill under the CS7 protected composite (one-time per photo, cached, flag+kill respected, fail-open, cost-logged $0.10).
- Debug-driven fixes proven live on the olive photo (free standalone `garment_prep` actions + `textScrub` debug trail in the job result): Gemini needs the canonical **box_2d** format (custom JSON shapes → empty); **2.5-flash truncates at MAX_TOKENS because thinking tokens count** (fix: `thinkingBudget: 0` + 4096 + truncation-tolerant box_2d regex salvage; 2.0-flash 404s on this key); **FLUX Fill paints instruction prompts literally** ("Remove the marketing…" appeared as rendered text — prompts must DESCRIBE the desired content).
- Final live E2E (run 6): prep crop fully text-free (golden Bangla + white lines + plates all gone), adult + child + protected composite clean of ALL marketing text. Residual: tiny partial glyph at the crop's right edge (box clipped); child garment hem length varies shot-to-shot in PREVIEW mode (1 gen, QC 3/5 passes by design) — Production mode's hard gate is the owner's quality lever.
