    ...
    try {
      ...
      return {
        ...
        data: {
          enabled,
          message: enabled
            ? 'নেটিভ অ্যাপ পুশ চালু করলাম, Sir — এখন এজেন্টের অ্যালার্ট আপনার ফোনের অ্যাপেও যাবে। ' +
              'চাইলে "টেস্ট পুশ পাঠাও" বললে একটা পরীক্ষা নোটিফিকেশন পাঠাব।'
            : 'নেটিভ অ্যাপ পুশ বন্ধ করলাম, Sir — এখন শুধু ntfy/টেলিগ্রামে অ্যালার্ট যাবে।',
        },
      }
    } catch (err) {
    ...
    try {
      if (!(await isAgentNativePushEnabled())) {
        return {
          ...
          data: {
            message:
              'নেটিভ অ্যাপ পুশ এখন বন্ধ আছে, Sir। আগে চালু করতে বলুন — "নেটিভ পুশ চালু করো"।',
          },
        }
      }
      ...
      if (!ownerIds.length) {
        return {
          ...
          data: {
            message:
              'আপনার অ্যাপ-অ্যাকাউন্ট (ERP user) খুঁজে পেলাম না, Sir — তাই পুশ পাঠানোর ঠিকানা নেই। ' +
      ...
      const msg = String(input.message ?? '').trim() ||
        'এটি একটি টেস্ট নোটিফিকেশন, Sir — আপনার ফোনের অ্যাপে এলে বুঝবেন নেটিভ পুশ ঠিকঠাক কাজ করছে।'
      ...
      return {
        ...
        data: {
          ...
          message: res.ok
            ? 'টেস্ট পুশ পাঠিয়ে দিয়েছি, Sir — আপনার ফোনের অ্যাপে নোটিফিকেশনটা এলো কিনা একটু দেখুন। ' +
              'এলে বুঝব Android-এ নেটিভ পুশ পুরোপুরি কাজ করছে।'
            : `টেস্ট পুশ পাঠাতে পারলাম না, Sir (${res.reason ?? 'অজানা সমস্যা'})।`,
        },
      }
    } catch (err) {
      return { success: false, error: String(err) }
