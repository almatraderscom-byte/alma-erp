...
function ArchiveBadge({ date }: { date: string }) {
  return (
    <span className="inline-flex items-center gap-1.5 rounded-full border border-zinc-300 bg-zinc-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-muted">
  ...
  return (
    <div className="mt-1.5">
      <div className={cn(!expanded && needsMore && 'line-clamp-2')}>
        {expanded || !needsMore ? (
          <pre className="whitespace-pre-wrap font-sans text-[11px] leading-relaxed text-cream/90">{text}</pre>
        ) : (
          <span className="text-[11px] text-cream/90">{text.slice(0, 120)}…</span>
        )}
      </div>
      {needsMore && (
...
function AckBadge({ m }: { m: StaffMonitorRow }) {
  ...
  if (m.status === 'queued' || m.status === 'pending') return <span className="inline-flex items-center rounded-md border border-zinc-300 bg-zinc-50 px-1.5 py-0.5 text-[9px] text-muted">sending…</span>
  return null
}
  ...
  return (
    <div className="mx-auto flex max-w-7xl gap-0 pb-[calc(4.5rem+env(safe-area-inset-bottom))] md:pb-8 lg:gap-4 lg:p-4">
      <div className="min-w-0 flex-1 space-y-3 p-3 sm:p-4">
        ...
        <div
          className="sticky top-0 z-30 -mx-3 border-b border-border-subtle bg-transparent/90 px-3 pb-2.5 backdrop-blur-md sm:-mx-4 sm:px-4"
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex items-center gap-3">
              <h1 className="text-xl font-black tracking-tight text-cream">CC Camera Room</h1>
          ...
          <div className="flex shrink-0 flex-wrap items-center gap-2">
            <div className="inline-flex rounded-xl border border-border-subtle bg-transparent p-0.5">
              {(['ALL', 'ALMA_LIFESTYLE', 'ALMA_TRADING'] as const).map(b => (
                <button key={b} type="button" onClick={() => setBusinessFilter(b)}
                  className={cn('rounded-lg px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider transition-all',
                    businessFilter === b ? 'bg-[#E07A5F]/12 text-[#E07A5F] shadow-sm' : 'text-muted hover:text-muted')}>
            ...
            {viewingHistory ? (
              ...
              <button type="button" onClick={() => void loadLive(true)} disabled={syncing}
                className={cn('rounded-xl border px-3 py-1.5 text-[10px] font-bold transition-all',
                  syncing ? 'border-border-subtle text-muted' : 'border-[#E07A5F]/25 bg-[#E07A5F]/[0.08] text-[#E07A5F] hover:bg-[#E07A5F]/15')}>
                {syncing ? '…' : '↻ Sync'}
              </button>
            )}
          ...
          <motion.div className="space-y-3" variants={staggerContainer} initial="hidden" animate="show">
                ...
                <SectionCard title="Live Surveillance" icon="📡" accent="red">
                  <div className="mb-3">
                    <div className="mb-1.5 flex items-center justify-between gap-2">
                      <h4 className="text-[10px] font-bold uppercase tracking-wider text-muted">📍 Geo-Fence</h4>
                      <button
                        type="button"
                        ...
                        className={cn(
                          ...
                          (displayData.geoFenceMonitoringEnabled ?? true)
                            ...
                            : 'border-zinc-300 bg-zinc-100 text-muted-hi',
                    ...
                    {(displayData.geoFenceMonitoringEnabled ?? true) ? (
                      displayData.geoStatus?.length ? (
                        <div className="flex flex-wrap gap-2">
                          {displayData.geoStatus.map((g) => (
                            <div key={g.staffId} className={cn(
                              ...
                              g.status === 'no_data' && 'border-border bg-zinc-50 text-muted',
                            )}>
                        ...
                        <p className="text-[11px] text-muted">কোনো staff location ডেটা নেই।</p>
                      )
                    ) : (
                      <p className="rounded-lg border border-border bg-zinc-50 px-2.5 py-2 text-[11px] text-muted-hi">
                  ...
                  {displayData.productivityAlerts?.length ? (
                    <div>
                      <h4 className="mb-1.5 text-[10px] font-bold uppercase tracking-wider text-muted">⚡ Productivity</h4>
                      <div className="space-y-1">
                        {displayData.productivityAlerts.map((a, i) => (
                          <div key={i} className={cn(
                            ...
                            <span className="min-w-0 flex-1 text-muted-hi">{a.message}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {staffToggleDefs.length > 0 && (
                    <div className="mt-3 border-t border-border-subtle pt-3">
                      <h4 className="mb-1.5 text-[10px] font-bold uppercase tracking-wider text-muted">🎛️ Staff Task Controls</h4>
                      <div className="space-y-1.5">
                        {staffToggleDefs.map((def) => {
                          const enabled = staffToggles[def.key] !== false
                          return (
                            <div key={def.key} className="flex items-center justify-between gap-2 rounded-lg border border-border-subtle bg-transparent px-2.5 py-2">
                              <div className="min-w-0">
                                <p className="text-[11px] font-bold text-cream/80">{def.label}</p>
                                <p className="mt-0.5 text-[10px] leading-tight text-muted">{def.hint}</p>
                              </div>
                              <button
                                type="button"
                  ...
                  actions={
                    <button type="button" disabled={healthScanning} onClick={() => void loadHealthScan()}
                      className={cn('rounded-lg border px-2 py-1 text-[9px] font-bold transition-all',
                        healthScanning ? 'border-border-subtle text-muted' : 'border-[#E07A5F]/25 bg-[#E07A5F]/[0.08] text-[#E07A5F] hover:bg-[#E07A5F]/15')}>
                      {healthScanning ? '⏳ Scanning…' : '🔍 Scan Now'}
                    </button>
                  }
                >
                  {!healthReport && !healthScanError ? (
                    <p className="py-2 text-[10px] text-muted">{healthScanning ? 'Scanning…' : 'Loading health scan…'}</p>
                        ...
                        <div key={i} className={cn('rounded-lg border p-2 text-[11px]',
                          ...
                          <div className="flex items-center gap-2">
                            <span className={cn('shrink-0 rounded px-1 py-0.5 text-[8px] font-bold uppercase',
                              ...
                              issue.severity === 'medium' ? 'bg-amber-100 text-amber-600' : 'bg-zinc-100 text-muted')}>
                              {issue.severity}
                            </span>
                            <span className="min-w-0 flex-1 truncate font-semibold text-cream/80">{issue.title}</span>
                          </div>
                          <div className="mt-1 flex items-center justify-between">
                            <p className="text-[10px] text-muted">{issue.detail}</p>
                            {issue.severity === 'high' && isAutoFixEligible(issue) && (
                              <button type="button" disabled={fixingIssue === issue.title} onClick={() => void requestFix(issue)}
                                className={cn('ml-2 shrink-0 rounded-md border px-2 py-0.5 text-[9px] font-bold transition-all',
                                  fixingIssue === issue.title ? 'border-border-subtle text-muted' : 'border-[#81B29A]/30 bg-[#81B29A]/[0.08] text-[#81B29A] hover:bg-[#81B29A]/15')}>
                                {fixingIssue === issue.title ? '⏳...' : '🤖 Fix This'}
                              </button>
                            )}
                            {issue.severity === 'high' && !isAutoFixEligible(issue) && (
                              <span className="ml-2 shrink-0 text-[9px] font-medium text-muted">Agent tool দিয়ে ঠিক করুন</span>
                            )}
                          </div>
                        </div>
                      ))}
                ...
                <SectionCard title={`Auto-Fix Pipeline (${autoFixActions.length})`} icon="🤖" accent="teal"
                  ...
                  <div className="space-y-2">
                    {autoFixActions.map(a => {
                      const sc = a.status === 'pending' ? 'text-[#D4A84B] bg-[#D4A84B]/10' : a.status === 'approved' || a.status === 'in_progress' ? 'text-[#81B29A] bg-[#81B29A]/10' : a.status === 'completed' ? 'text-emerald-600 bg-emerald-50' : a.status === 'rejected' ? 'text-muted bg-zinc-100' : 'text-red-600 bg-red-50'
                      ...
                      return (
                        <div key={a.id} className="rounded-lg border border-border-subtle bg-transparent p-2.5 text-[11px]">
                          <div className="flex items-center gap-2">
                            <span className={cn('shrink-0 rounded px-1.5 py-0.5 text-[8px] font-bold', sc)}>{sl}</span>
                            <span className="font-semibold text-cream/80 truncate">{a.payload?.title ?? 'Unknown'}</span>
                            <span className="ml-auto text-[9px] text-muted">${(a.costEstimate ?? 0).toFixed(2)}</span>
                          </div>
                          {a.status === 'pending' && (
                ...
                <SectionCard title={`Background Services (${displayData.continuousServices.length})`} icon="⚡" accent="teal">
                  <div className="flex flex-wrap gap-2">
                    {displayData.continuousServices.map(s => (
                      <span key={s.key} className="inline-flex items-center gap-1.5 rounded-lg border border-border-subtle bg-transparent px-2 py-1 text-[10px] text-muted">
                    ...
                    {displayData.unackedMessages.map(m => (
                      <div key={m.id} className="rounded-lg border border-[#D4A84B]/15 bg-[#D4A84B]/[0.04] p-2 text-[11px]">
                        <div className="flex items-center justify-between gap-2">
                          <span className="truncate font-medium text-cream/90">{m.staffName ?? '—'}</span>
                          <div className="flex shrink-0 items-center gap-1.5">
                            <span className="rounded bg-[#D4A84B]/10 px-1 py-0.5 text-[9px] font-semibold text-[#D4A84B]">{typeLabel(m.type)}</span>
                            <span className="tabular-nums text-[10px] text-muted">{m.sentAt ? fmtTime(m.sentAt) : fmtTime(m.createdAt)}</span>
                          </div>
                        </div>
                        ...
                        <div className="mt-1.5 flex items-center justify-end">
                          <button type="button" disabled={escalating === m.id} onClick={() => handleEscalate(m)}
                            className={cn('rounded-md border px-2 py-1 text-[9px] font-bold transition-all',
                              escalating === m.id ? 'border-border-subtle text-muted' : 'border-red-500/25 bg-red-50 text-red-600 hover:bg-red-100')}>
                ...
                <SectionCard title="Pending Approvals (48h)" icon="⏳" accent="amber"
                  ...
                  <div className="space-y-1.5">
                    {displayData.pendingApprovals!.map(a => {
                      ...
                      const ageColor = ageH > 12 ? 'border-red-500/20 bg-red-50' : ageH > 2 ? 'border-[#D4A84B]/20 bg-[#D4A84B]/[0.04]' : 'border-border-subtle bg-transparent'
                      return (
                        <div key={a.id} className={cn('rounded-lg border px-3 py-2', ageColor)}>
                          <div className="flex items-start justify-between gap-2">
                            <div className="min-w-0 flex-1">
                              <p className="text-[10px] font-bold text-muted">{a.type.replace(/_/g, ' ')}</p>
                              <p className="mt-0.5 text-[11px] text-muted line-clamp-2">{a.summary}</p>
                              <span className="text-[9px] text-muted">{ageH < 1 ? `${Math.round(ageH * 60)}m ago` : `${ageH.toFixed(1)}h ago`}</span>
                            </div>
            ...
            <motion.div variants={fadeIn}>
              <SectionCard title="Message Feed" icon="📨" accent="teal">
                {feedItems.length === 0 ? (
                  <p className="py-4 text-center text-[11px] text-muted">কোনো মেসেজ লগ নেই</p>
                ) : (
                  <>
                    <div className="space-y-1.5">
                      {visibleFeed.map(m => (
                        <div key={m.id} className={cn('flex items-start gap-2.5 rounded-lg px-2.5 py-2 text-[11px] transition-all',
                          ...
                          m.status === 'delivered' ? 'bg-transparent border-l-2 border-l-emerald-500/30' :
                          'bg-transparent border-l-2 border-l-amber-500/25')}>
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                              <span className="min-w-0 flex-1 truncate font-medium text-cream/70">{m.staffName ?? '—'}</span>
                              <span className="shrink-0 rounded bg-white/[0.04] px-1 py-0.5 text-[9px] font-semibold text-muted">{typeLabel(m.type)}</span>
                              <span className="shrink-0"><AckBadge m={m} /></span>
                              <span className="shrink-0 tabular-nums text-[10px] text-muted">{fmtTime(m.createdAt)}</span>
                            </div>
                            <FeedMessage content={m.content ?? ''} />
                          </div>
                        </div>
                    ...
                    {feedItems.length > 6 && (
                      <button type="button" onClick={() => setFeedExpanded(v => !v)}
                        className="mt-2 w-full rounded-lg border border-border-subtle bg-transparent py-2 text-[10px] font-semibold text-muted transition-all hover:text-muted hover:border-[#E07A5F]/15">
              ...
              <motion.div variants={fadeIn}>
                <SectionCard title="Live Tasks & Reminders" icon="📌" accent="teal">
                  {(displayData.activeReminders?.length ?? 0) > 0 && (
                    <div className="mb-3">
                      <p className="text-[9px] font-bold uppercase tracking-wider text-muted mb-1.5">Reminders</p>
                      <div className="space-y-1">
                        {displayData.activeReminders!.map(r => (
                          <div key={r.id} className="flex items-center justify-between gap-2 rounded-lg border border-border-subtle bg-transparent px-2.5 py-1.5">
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-1.5">
                                <span className={cn('inline-block h-1.5 w-1.5 rounded-full', r.tier >= 3 ? 'bg-red-500' : r.tier >= 2 ? 'bg-amber-500' : 'bg-[#81B29A]')} />
                                <span className="truncate text-[11px] font-medium text-cream/70">{r.title}</span>
                              </div>
                            </div>
                  ...
                  {(displayData.activeTodos?.length ?? 0) > 0 && (
                    <div>
                      <p className="text-[9px] font-bold uppercase tracking-wider text-muted mb-1.5">Owner Todos</p>
                      <div className="space-y-1">
                        {displayData.activeTodos!.map(t => (
                          <div key={t.id} className="flex items-center justify-between gap-2 rounded-lg border border-border-subtle bg-transparent px-2.5 py-1.5">
                            <span className="text-[11px] font-medium text-cream/70 truncate">{t.title}</span>
                            <span className={cn('rounded px-1.5 py-0.5 text-[9px] font-bold',
                              t.priority === 'high' ? 'bg-red-50 text-red-600' : t.priority === 'urgent' ? 'bg-red-100 text-red-700' : 'bg-zinc-100 text-muted')}>
                              {t.priority}
                            </span>
                          </div>
                        ))}
            ...
            <motion.div variants={fadeIn}>
              <SectionCard title="Agent Capabilities" icon="🧠" accent="coral"
                actions={<button type="button" onClick={() => setCapsOpen(v => !v)} className="text-[10px] font-bold text-muted hover:text-muted transition-colors">{capsOpen ? '▴ Hide' : '▾ Show'}</button>}>
                <AnimatePresence>
                  {capsOpen ? (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        {AGENT_CAPABILITIES.map(cap => (
                          <div key={cap.category} className="rounded-xl border border-border-subtle bg-transparent p-3">
                            <div className="mb-2 flex items-center gap-2">
                              <span>{cap.icon}</span>
                              <span className="text-[11px] font-bold text-cream/70">{cap.category}</span>
                            </div>
                              ...
                              {cap.items.map(item => (
                                <li key={item.name} className="flex items-start gap-1.5">
                                  ...
                                  <div>
                                    ...
                                    <div className="text-[10px] text-muted mt-0.5">💬 {item.command}</div>
                                  </div>
                                </li>
                              ))}
                            </ul>
                    ...
                    <p className="text-[10px] text-muted">9 categories · ~100+ tools — ERP, Staff, Trading, Finance, Content, CS, Website, Personal, Diagnostics</p>
                  )}
                </AnimatePresence>
              </SectionCard>
            </motion.div>
      ...
      <aside className={cn(
        ...
        <div className="mb-3 flex items-center justify-between">
          ...
          <button type="button" onClick={() => setHistoryOpen(false)} className="text-xs text-muted hover:text-muted lg:hidden">✕</button>
        </div>
        <p className="mb-3 text-[10px] text-muted">Last {liveData.feedDays ?? 7} days</p>
        <div className="space-y-0.5">
          <button type="button" onClick={() => { setSelectedDate(null); setHistoryData(null); setHistoryOpen(false) }}
            className={cn('w-full rounded-lg px-2.5 py-2 text-left text-[11px] font-medium transition-all',
              !viewingHistory ? 'bg-[#E07A5F]/10 text-[#E07A5F]' : 'text-muted hover:bg-white/[0.02] hover:text-muted')}>
            Today (live)
          </button>
          {(liveData.historyDates ?? []).map(date => (
            <button key={date} type="button" onClick={() => { void loadHistoryDay(date); setHistoryOpen(false) }}
              className={cn('w-full rounded-lg px-2.5 py-2 text-left text-[11px] font-medium tabular-nums transition-all',
                selectedDate === date ? 'bg-white/[0.04] text-cream/80' : 'text-muted hover:bg-white/[0.02] hover:text-muted')}>
