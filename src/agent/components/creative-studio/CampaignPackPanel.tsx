    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-28 pt-3">
      <div className="mx-auto max-w-2xl space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <label className="flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-[11px] text-cream">
            Family version
            <input
              type="checkbox"
        ...
        {loading && <div className="rounded-2xl bg-white/[0.04] p-5 text-center text-[11px] text-muted">Manifest তৈরি হচ্ছে…</div>}
        {error && <p role="alert" className="rounded-xl border border-red-400/20 bg-red-400/10 p-3 text-[11px] text-red-200">{error}</p>}
        ...
        {manifest && (
          <>
            <section className="rounded-2xl border border-white/10 bg-white/[0.035] p-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="text-right">
                  ...
                  <p className="text-[11px] text-muted">hard cap ${manifest.hardCostCeilingUsd.toFixed(2)}</p>
                </div>
              </div>
            ...
            <section className="space-y-1.5">
              {manifest.stages.map((stage, index) => (
                <div key={stage.id} className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-2.5 py-2">
                  <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-white/[0.07] text-[11px] text-muted">{index + 1}</span>
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-[11px] font-bold text-cream">{stage.labelBn}</p>
                    <p className="truncate text-[11px] text-muted">{stage.engine} · ~{stage.estimatedSeconds}s</p>
                  </div>
                  <span className={stage.paid ? 'text-[11px] font-bold text-amber-200' : 'text-[11px] font-bold text-emerald-200'}>
            ...
            {manifest.requiresPaidConfirmation && (
              <label className="flex items-start gap-2 rounded-xl border border-amber-300/25 bg-amber-300/[0.08] p-3 text-[11px] leading-relaxed text-amber-100">
                <input
                  type="checkbox"
