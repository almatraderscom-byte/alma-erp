...
const POLICY_LABEL: Record<string, { text: string; className: string }> = {
  green: { text: 'নিজে চলেছে', className: 'bg-emerald-500/15 text-emerald-700' },
  amber: { text: 'আপনি অনুমতি দিয়েছেন', className: 'bg-amber-500/15 text-amber-700' },
}
  ...
  return (
    <div className="space-y-4 px-4 pb-8">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <button
            ...
            className={`shrink-0 rounded-xl px-4 py-2 text-sm font-medium text-white disabled:opacity-50 ${
              enabled ? 'bg-emerald-600' : 'bg-black/30'
            }`}
          >
      ...
      <section className="rounded-2xl border border-black/10 bg-white/80 p-4 backdrop-blur">
        ...
        {pairCode && (
          <div className="mt-3 rounded-xl border border-[#E07A5F]/40 bg-[#E07A5F]/10 p-3">
            <p className="text-sm text-black/70">Mac-এ Terminal খুলে একবার চালান (কোড ১০ মিনিট চলবে):</p>
        ...
        <div className="mt-3 space-y-2">
          {devices.length === 0 && <p className="text-sm text-black/50">এখনো কোনো Mac যুক্ত নেই।</p>}
          {devices.map((d) => (
            <div key={d.id} className="flex items-center justify-between gap-3 rounded-xl bg-black/[0.04] px-3 py-2">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className={`h-2 w-2 shrink-0 rounded-full ${d.online ? 'bg-emerald-400' : 'bg-black/25'}`} />
              ...
              <button
                ...
                className="shrink-0 rounded-lg border border-black/15 px-3 py-1.5 text-xs text-black/70 disabled:opacity-50"
              >
                সরিয়ে দিন
              </button>
            </div>
      ...
      <section className="rounded-2xl border border-red-500/25 bg-red-500/5 p-4 backdrop-blur">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h2 className="text-base font-semibold text-red-700">সব থামান</h2>
            <p className="mt-1 text-sm text-black/60">
      ...
      <section className="rounded-2xl border border-black/10 bg-white/80 p-4 backdrop-blur">
        ...
        <div className="mt-3 space-y-2">
          {(data?.history ?? []).length === 0 && <p className="text-sm text-black/50">এখনো কিছু চালানো হয়নি।</p>}
          {(data?.history ?? []).map((h) => {
            ...
            return (
              <div key={h.id} className="rounded-xl bg-black/[0.04] px-3 py-2">
                <div className="flex items-start justify-between gap-3">
                  <code className="min-w-0 break-all text-xs text-black/80">{h.command ?? h.action}</code>
