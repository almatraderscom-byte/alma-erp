...
function serverState(lifecycle: StudioV3Lifecycle) {
  ...
  if (lifecycle === 'review') return 'qc_failed' as const
  return 'all' as const
}
...
export function StudioV3Gallery({
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    setIssues([])
    const wantsGallery = ['all', 'image', 'video', 'audio'].includes(category)
    ...
    if (category === 'all' || category === 'voice') { requests.push(port.listVoices(activeBrand?.brandProfileId)); keys.push('voice') }
    if (category === 'all' || category === 'avatar') { requests.push(port.listModels(activeBrand?.brandProfileId)); keys.push('avatar') }
    if (category === 'all' || category === 'project') { requests.push(port.listProjects(activeBrand?.brandProfileId)); keys.push('project') }
    ...
    setLoading(false)
  }, [activeBrand?.brandProfileId, category, lifecycle, port, search])

  useEffect(() => {
    void load()
    return () => { loadSequence.current += 1 }
  }, [activeBrand?.brandProfileId, load])

  const loadMore = async () => {
    if (!nextCursor || loadingMore || !['all', 'image', 'video', 'audio'].includes(category)) return
    setLoadingMore(true)
    try {
  ...
  }, [aspectFilter, category, items, lifecycle, providerFilter, search, sort])
    ...
    <div className={styles.page}>
      ...
      <section aria-label="Gallery controls" className={styles.galleryControls}>
        <div className={styles.galleryTabs}>
          {STUDIO_V3_GALLERY_CATEGORIES.map((item) => <button aria-pressed={category === item.id} key={item.id} onClick={() => selectCategory(item.id)} type="button">{item.label}</button>)}
        </div>
        <div className={styles.lifecycleTabs}>
          {STUDIO_V3_LIFECYCLES.map((item) => <button aria-pressed={lifecycle === item.id} key={item.id} onClick={() => setLifecycle(item.id)} type="button">{item.label}</button>)}
        </div>
        <form className={styles.gallerySearch} onSubmit={(event) => { event.preventDefault(); setSearch(searchDraft.trim()) }}>
          <StudioV3Icon name="search" />
          <input aria-label="Search Gallery" onChange={(event) => setSearchDraft(event.target.value)} placeholder="Search title, product code, mode or provider" type="search" value={searchDraft} />
          <button type="submit">Search</button>
        </form>
        <div className={styles.galleryFilters}>
          <label><StudioV3Icon name="filter" /><span className="sr-only">Provider filter</span><select aria-label="Provider filter" onChange={(event) => setProviderFilter(event.target.value)} value={providerFilter}><option value="all">All providers</option>{providers.map((value) => <option key={value}>{value}</option>)}</select></label>
          <label><span className="sr-only">Aspect filter</span><select aria-label="Aspect filter" onChange={(event) => setAspectFilter(event.target.value)} value={aspectFilter}><option value="all">All aspects</option>{aspects.map((value) => <option key={value}>{value}</option>)}</select></label>
          <label><span className="sr-only">Sort Gallery</span><select aria-label="Sort Gallery" onChange={(event) => setSort(event.target.value as typeof sort)} value={sort}><option value="newest">Newest</option><option value="oldest">Oldest</option><option value="name">Name</option><option value="cost">Cost</option></select></label>
      ...
      {lifecycle === 'archived' && (
        <div className={styles.inlineWarning} role="status">
          ...
          <span>The current production API exposes retention statistics but not a complete archive listing. Only records with an explicit deleted/archived lifecycle are shown; the Foundation archive-list adapter remains required.</span>
        </div>
      )}
      <p className={styles.scopeNotice}>
        <StudioV3Icon name="lock" />
        Brand switch reloaded every lane. Projects are presentation-filtered by canonical brand ID
        after a successful owner response, and recipes send the active brand to their owner-only
        server filter. Media, avatars and voices expose no brand key ({STUDIO_V3_SCOPE_BOUNDARY.gallery}).
      </p>
