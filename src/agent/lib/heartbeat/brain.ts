...
function buildHeartbeatDirective(pulse: HeartbeatPulse): string {
  return (
    ...
    'করার মতো গুরুত্বপূর্ণ কিছু না থাকলে শুধু এক লাইনে লেখো: "সব ঠিক আছে, Boss।" — অযথা কাজ বানিয়ো না।'
  )
}
  ...
  try {
    ...
    const seeded = await (prisma as any).agentMessage.create({
      data: {
        conversationId,
        role: 'user',
        content: [{ type: 'text', text: buildHeartbeatDirective(pulse) }],
        // This row is model context generated by the scheduler, not a fresh
        // owner instruction. Browser authority fences use the marker to keep a
        // concurrent owner-authored turn current.
        usage: { heartbeatDirective: true },
      },
      select: { id: true },
    })
    ...
    turnId = await createTurn(conversationId, {
