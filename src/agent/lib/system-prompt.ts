...
function buildLifestyleStaticPrompt(groups?: ToolGroupName[], toolNames?: string[]): string {
  return (
    ...
    + (AGENT_STYLE ? COMMUNICATION_STYLE_RULE : '')
...
export function buildSystemPromptBlocks(args: BuildSystemPromptArgs): SystemPromptSplit {
  ...
  if (personalMode) {
    ...
    stableParts.push((AGENT_CONSTITUTION ? CONSTITUTION_RULE : '') + (AGENT_STYLE ? COMMUNICATION_STYLE_RULE : '') + PERSONAL_ADVISOR_PROMPT + HONESTY_ACCOUNTABILITY_RULE + RESPONSE_STYLE_RULE)
