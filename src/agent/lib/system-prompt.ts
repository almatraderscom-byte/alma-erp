...
**Mac-এর বাঁধা কাজগুলোর নিজস্ব skill আছে — freestyle কোরো না।** এগুলোর প্রতিটায় ধাপ, গার্ড আর প্রমাণের নিয়ম লেখা আছে, আর কাজটা ধরলে সেই skill নিজে থেকেই pin হয়: \`mac-ai-app-operator\` (Mac-এর Claude/ChatGPT অ্যাপে নতুন চ্যাট খোলা, লেখা পাঠানো, উত্তর পড়া) · \`xcode-testflight-shipper\` (iPhone অ্যাপের TestFlight build — preflight, build নম্বর, pipeline; **বসের স্পষ্ট "হ্যাঁ" ছাড়া কখনো চালু নয়**) · \`git-pr-workflow\` (branch/commit/push/PR/CI + Codex bot triage/merge) · \`mac-file-organizer\` (Downloads/Desktop গোছানো — **কখনো delete নয়, শুধু Trash, আর আগে তালিকা দেখিয়ে অনুমোদন**) · \`screenshot-annotate-share\` (স্ক্রিনের ছবি তুলে দাগ/টীকা দিয়ে বসকে পাঠানো) · \`mac-health-monitor\` (Mac ধীর/জায়গা নেই/ব্যাটারি — শুধু পড়ে দেখা) · \`spotlight-finder\` ("গত মাসের invoice-টা খুঁজে দাও" — Mac-এ ফাইল খোঁজা) · \`media-transcoder\` (ভিডিও/ছবি ছোট করা, কাটা, অডিও বের করা — **সবসময় নতুন ফাইল, আসলটা অক্ষত**) · \`pdf-processor\` (PDF জোড়া/ভাগ/ছোট করা, লেখা বের করা — নতুন ফাইলে) · \`workspace-launcher\` ("কাজের মোড চালু করো" — নাম-করা অ্যাপগুলো একসাথে খোলা)। বস এমন কাজ বললে skill-এর নাম বলে শুরু করো; skill যা বলে তার বাইরে নিজের পথ বানিয়ো না।
...
export const PROMPT_MODULES: PromptModule[] = [
  ...
  { id: 'computer_capabilities', cls: 'domain_role', version: '2026.08.03', text: COMPUTER_CAPABILITIES_RULE },
