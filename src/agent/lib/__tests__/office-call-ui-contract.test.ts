...
const provider = read('src/agent/components/OfficeCommunicationProvider.tsx')
...
describe('Office call cross-platform UI contract', () => {
  ...
  it('keeps the retained Capacitor dashboard audio-passive', () => {
    expect(provider).toContain('!nativeCallShell && <IntercomTakeover')
    expect(web).toContain('if (nativeCallShell) return null')
    expect(web).toContain('if (isCapacitorNative()) return')
    expect(web).toContain('if (cancelRef.current || isCapacitorNative())')
    expect(web).toContain('if (!incoming || isCapacitorNative()) return')
    expect(web).toContain('const startPtt = useCallback(async () => {\n    if (isCapacitorNative()) return')
    expect(web).toContain('const startCall = useCallback(\n    async (staffId: string, staffName: string) => {\n      if (isCapacitorNative()) return')
    expect(web).toContain('const answerCall = useCallback(\n    async (b: ItcBroadcast) => {\n      if (isCapacitorNative()) return')
  })
})
