...
describe('native voice upload contract', () => {
  ...
  it('discriminates loudspeaker echo while preserving natural barge-in', () => {
    ...
    expect(voice).toContain('"endOfSpeechSensitivity": "END_SENSITIVITY_HIGH"')
    expect(voice).toContain('"silenceDurationMs": 800')
  ...
  it('gates CallKit media on real activation and keeps receiver routing possible', () => {
    ...
    expect(voice).toContain('AlmaLiveVoiceLifecycleSessionFence.acceptsSourceToken(')
    expect(voice).toContain('endFromLifecycleRequest(')
