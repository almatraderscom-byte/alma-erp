...
export const newOrderInputCls = (err?: string) =>
  `w-full bg-black/[0.03] border rounded-xl px-3 py-2.5 text-sm text-cream placeholder-zinc-600 focus:outline-none transition-colors ${
...
export const newOrderSelectCls = (err?: string) =>
  `w-full bg-black/[0.03] border rounded-xl px-3 py-2.5 text-sm text-cream focus:outline-none transition-colors appearance-none cursor-pointer ${
    err ? 'border-red-400/60 focus:border-red-400' : 'border-border focus:border-gold-dim/70'
  }`
