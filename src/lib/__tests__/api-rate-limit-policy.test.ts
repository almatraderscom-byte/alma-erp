...
describe('computer preview API rate buckets', () => {
  it('isolates each high-frequency paired-device path from a shared NAT bucket', () => {
    ...
    const browserAuthorize = apiRateLimitPolicy('/api/assistant/live-browser/authorize')
    ...
    expect(browserAuthorize).toEqual({
      bucket: 'device-api:/api/assistant/live-browser/authorize', limit: 720,
    })
