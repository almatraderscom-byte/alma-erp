...
export async function resolveLifecycleCompositionPin(
  actor: StudioActor,
  input: {
    brandProfileId: string
    projectId: string
  ...
  assertLifecycleRuntimeEnabled()
  const access = await requireStudioBrandAccess(actor, input.brandProfileId)
  const [project, composition] = await Promise.all([
    db.creativeProject.findUnique({ where: { id: input.projectId } }),
    db.creativeComposition.findUnique({ where: { id: input.compositionId } }),
  ])
  if (
    !project
    || project.id !== input.projectId
  ...
  if (
    ...
    || composition.projectId !== input.projectId
  ) throw new LifecycleServiceError('composition_scope_mismatch', 403)
  return createStudioFoundationLifecyclePort(actor).getCompositionPin({
    actorId: actor.userId,
