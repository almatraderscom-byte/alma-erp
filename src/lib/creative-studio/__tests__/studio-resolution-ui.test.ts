...
describe('Creative Studio final-output resolution UI', () => {
  it('derives the resolution contract from the exact snapshotted generic model', () => {
    expect(resolutionEngineForGenericModel('gemini-3-pro-image')).toBe('gemini')
    expect(resolutionEngineForGenericModel('gpt-image-2')).toBe('gpt')
    expect(resolutionEngineForGenericModel('seedream-5.0-pro')).toBe('seedream')

    const state = buildStudioResolutionUiState(
      {
        ...base,
        mode: 'try_on',
        imageEngine: resolutionEngineForGenericModel('seedream-5.0-pro'),
      },
