...
function Row({
  ...
  const tone =
    ok === null ? 'text-zinc-500' : ok ? 'text-green-400' : 'text-red-400'
    ...
    <div className="flex justify-between gap-4 py-2 border-b border-border/60 last:border-0">
      <div className="flex items-start gap-2 min-w-0">
        ...
        <div className="min-w-0">
          ...
          {detail ? <p className="text-[10px] text-zinc-500 font-mono truncate">{detail}</p> : null}
        </div>
      </div>
      ...
      <div className="p-4 md:p-6 max-w-xl space-y-4">
        <Card className="p-5 border-gold-dim/25 bg-[#0c0c10] space-y-1">
          ...
          <p className="text-[11px] text-zinc-500 leading-snug">
            ...
            <span className="font-mono text-zinc-400">NEXT_PUBLIC_API_URL</span>
            ).
          </p>
            ...
            <>
              ...
              {typeof data.userRowCount === 'number' ? (
                <p className="text-[10px] text-zinc-500 pt-1">
                  User rows in DB: <span className="font-mono text-zinc-400">{data.userRowCount}</span>
                </p>
              ) : null}
            </>
          )}
        ...
        <Card className="p-5 space-y-2 text-[11px] text-zinc-500 leading-relaxed">
          <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-600">Quick fixes</p>
          <ul className="list-disc pl-4 space-y-1">
            <li>Copy the Supabase direct Postgres URI into both <span className="font-mono text-zinc-400">.env.local</span> and <span className="font-mono text-zinc-400">.env</span>.</li>
            <li>Run <span className="font-mono text-zinc-400">npx prisma db push</span> then <span className="font-mono text-zinc-400">npm run db:seed</span>.</li>
