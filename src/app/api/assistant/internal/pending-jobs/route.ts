...
export async function GET(req: NextRequest) {
  ...
  const jobs = await db.agentPendingAction.findMany({
    where: {
      ...
      type: { in: ['image_gen', 'video_gen', 'video_edit', 'long_agent_task', 'dispatch_staff_tasks', 'add_staff_task_now', 'staff_announcement', 'urgent_notify', 'outbound_call', 'browser_action', 'workbench_run'] },
    },
