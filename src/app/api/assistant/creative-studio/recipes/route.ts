...
function errorResponse(error: unknown) {
  ...
  console.error('[creative-recipes] request failed', error)
  return Response.json({ error: 'recipes_failed' }, { status: 500 })
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    return Response.json({ recipe: await createBrandRecipe(owner, body) }, { status: 201 })
  } catch (error) {
    return errorResponse(error)
  }
}
