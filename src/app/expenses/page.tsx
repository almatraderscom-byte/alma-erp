          ...
          {loading ? <Skeleton className="h-52" /> : (
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {Object.entries(byCat).sort((a, b) => b[1] - a[1]).slice(0, 12).map(([cat, amt]) => (
                <div key={cat} className="flex justify-between text-xs border-b border-white/[0.04] pb-2">
        ...
        {loading ? <Skeleton className="h-48" /> : expenses.length === 0 ? (
          ...
          <div className="overflow-x-auto min-w-0 max-w-full table-scroll max-h-[480px]">
            <table className="w-full min-w-[760px] text-left text-[11px]">
              <thead className="sticky top-0 bg-card/85 border-b border-white/[0.06]">
                <tr className="text-[11px] font-medium uppercase tracking-wider text-muted">
              ...
              <tbody>
                {expenses.map(er => (
                  <tr key={er.exp_id + er.date + er.amount} className="border-b border-white/[0.04] hover:bg-white/[0.04]/50 transition-colors">
                    <td className="py-2 pr-3 font-mono text-muted">{er.date.slice(0, 10)}</td>
            ...
            <form ref={expenseFormRef} id="add-expense-form" onSubmit={submit} className="flex min-h-0 flex-1 flex-col text-xs">
              <div className="mobile-modal-body space-y-3 px-5 pb-4">
                    <label className="block space-y-1">
                      ...
                      <input name="title" className="w-full rounded-xl bg-card/85 border border-white/[0.08] px-3 py-2 text-cream text-sm" />
                    </label>
                    <label className="block space-y-1">
                      <span className="text-muted">Category</span>
                      <select name="category" className="w-full rounded-xl bg-card/85 border border-white/[0.08] px-3 py-2 text-cream text-sm">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <label className="block space-y-1">
                        ...
                        <input name="amount" type="number" step="0.01" className="w-full rounded-xl bg-card/85 border border-white/[0.08] px-3 py-2 text-cream font-mono text-sm" />
                      </label>
                      <label className="block space-y-1">
                        <span className="text-muted">Payment date</span>
                        <input name="date" type="date" className="w-full rounded-xl bg-card/85 border border-white/[0.08] px-3 py-2 text-cream font-mono text-sm" />
                      </label>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <label className="block space-y-1">
                      ...
                      <label className="block space-y-1">
                        ...
                        <input name="payment_method" placeholder="bKash, bank…" className="w-full rounded-xl bg-card/85 border border-white/[0.08] px-3 py-2 text-cream text-sm" />
                      </label>
                    </div>
                    <label className="flex items-center gap-2 text-muted">
                      <input name="recurring" type="checkbox" className="rounded border-white/[0.08]" /> Recurring
                    </label>
                    <div className="space-y-2">
                      <span className="text-muted">Receipt / document</span>
                      <div
                        onDragOver={e => { e.preventDefault(); setDragActive(true) }}
                        onDragLeave={() => setDragActive(false)}
                        onDrop={handleDrop}
                        className={`rounded-2xl border border-dashed p-4 text-center transition-colors ${dragActive ? 'border-gold-dim/70 bg-gold/10' : 'border-white/[0.08] bg-white/[0.03]'}`}
                      >
                        <input
                          ref={fileInputRef}
                          type="file"
                        ...
                        {receipt ? (
                          <div className="space-y-2">
                            {receipt.contentType.startsWith('image/') ? (
                              <button type="button" onClick={() => openReceipt(receipt.url)} className="mx-auto block overflow-hidden rounded-xl border border-white/[0.08]">
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                <img src={receipt.url} alt="Receipt preview" className="max-h-36 max-w-full object-contain" />
                              </button>
                            ) : (
                              <button type="button" onClick={() => openReceipt(receipt.url)} className="rounded-xl border border-white/[0.08] bg-card/85 px-4 py-3 text-xs font-bold text-gold-lt">
                                PDF receipt · open/download
                              </button>
                            )}
                            <p className="text-[10px] text-muted">{receipt.fileName} · {(receipt.sizeBytes / 1024).toFixed(1)} KB</p>
                          ...
                          <div className="space-y-2">
                            <p className="text-xs text-muted">Drop receipt here, or upload/capture from mobile camera.</p>
                            <p className="text-[10px] text-muted">Images, screenshots, invoice scans, and PDF up to 10 MB.</p>
                            <Button size="xs" variant="gold" type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading}>
                              {uploading ? 'Uploading…' : 'Upload receipt'}
                            </Button>
                            {uploading && <div className="h-1.5 rounded-full bg-white/[0.08] overflow-hidden"><div className="h-full bg-gold" style={{ width: `${uploadProgress}%` }} /></div>}
                          </div>
                        )}
                      </div>
                    </div>
                    <label className="block space-y-1">
                      ...
                      <textarea name="notes" rows={3} className="w-full rounded-xl bg-card/85 border border-white/[0.08] px-3 py-2 text-cream text-sm" />
                    </label>
              </div>
