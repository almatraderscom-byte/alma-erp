...
9. **Session 2026-07-08 (b): queue items 1–13 DONE** (each parse-checked, committed, pushed; presentation-only, all metrics/tints/fallbacks preserved): **Orders** (`Ord*` hero: revenue + profit/orders split — shared `OrdersAurora`/`ordersGlass` primitives untouched), **Finance** (`Fin*`: P&L hero rev/net/exp/margin + 2×2 payroll tiles), **Invoices** (`Inv*`: totals hero amount+paid/unpaid+paid-share `InvMiniBar` + 3 count tiles + payment-tone `invBentoWash` on registry rows), **Payment accounts** (`Pay*`: counts-only hero, wallet cards untouched), **Inventory** (`Inven*`: stock-value hero + low/out tiles), **Activity** (per-source tinted icon chips + hairline connectors between day-group rows), **Analytics** (`An*`: revenue hero + AOV/delivery tiles + return-KPI tiles), **Insights** (`Ins*`: 30-day revenue hero with WoW trend colours + customer tiles; unused kpiCard removed), **Briefing** (light pass: hero glow `.blur(30)` → radial-gradient falloff), **Audit** (`Aud*`: total hero + OK/FAIL), **Attendance** (`Att*`: present hero + absent/late; KPI strip re-skinned with `attBentoWash`; check-in/face/GPS untouched), **Employees** (`Emp*`: team hero + active/roles), **CRM** (`Crm*`: lifetime-revenue hero + CLV/high-risk tiles).
