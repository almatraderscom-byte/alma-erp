# G07 Group Runner — Prompt Caching and Response Caching
...
1. `SPEC-061` — Stable-prefix hashing
2. `SPEC-062` — Provider prompt-cache adapter
3. `SPEC-063` — Cache-break diagnostics
4. `SPEC-064` — Conversation cache-key strategy
5. `SPEC-065` — Exact deterministic response cache
6. `SPEC-066` — Semantic read-only response cache
7. `SPEC-067` — Tool result cache with freshness
8. `SPEC-068` — Policy and permission cache exclusions
9. `SPEC-069` — Cross-tenant cache isolation
10. `SPEC-070` — Cache savings and correctness dashboard

## Prerequisites

- `G03` must have a merged PASS certification.
- `G05` must have a merged PASS certification.
- `G06` must have a merged PASS certification.

## Owned zones

- `src/agent/cache`
- `src/agent/providers/cache`
...
- create `artifacts/G07/GROUP_CERTIFICATION.md`
...
You are the sequential Group Runner for Prompt Caching and Response Caching.
Read GLOBAL_AGENT_CONTRACT.md, this RUNNER.md, and all ten specs.
Use one clean branch/worktree dedicated to G07.
