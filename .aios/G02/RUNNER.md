# G02 Group Runner — Request Admission Control Plane
...
1. `SPEC-011` — Single admission gateway
2. `SPEC-012` — Request source normalization
3. `SPEC-013` — Deterministic fast-path command router
4. `SPEC-014` — Structured task envelope
5. `SPEC-015` — Intent classification adapter
6. `SPEC-016` — Complexity classification
7. `SPEC-017` — Planning-need classification
8. `SPEC-018` — Risk admission classification
9. `SPEC-019` — Request deduplication and replay protection
10. `SPEC-020` — Admission bypass CI gate
...
- `src/agent/control-plane/admission`
- `src/app/api/agent`
...
- create `artifacts/G02/GROUP_CERTIFICATION.md`
...
You are the sequential Group Runner for Request Admission Control Plane.
Read GLOBAL_AGENT_CONTRACT.md, this RUNNER.md, and all ten specs.
Use one clean branch/worktree dedicated to G02.
