# G20 Group Runner — Observability Release and Continuous Optimization
...
1. `SPEC-191` — End-to-end trace model
2. `SPEC-192` — Agent operational SLOs
3. `SPEC-193` — Cost and quality dashboard
4. `SPEC-194` — Escalation and cache dashboard
5. `SPEC-195` — Shadow traffic framework
6. `SPEC-196` — Canary release framework
7. `SPEC-197` — Automatic rollback thresholds
8. `SPEC-198` — Model bake-off automation
9. `SPEC-199` — Workflow optimization recommendations
10. `SPEC-200` — Production readiness and final architecture certification

## Prerequisites

- `G07` must have a merged PASS certification.
- `G14` must have a merged PASS certification.
- `G19` must have a merged PASS certification.
...
- create `artifacts/G20/GROUP_CERTIFICATION.md`

## Required group certification

```text
Group: G20
Specs: SPEC-191..SPEC-200
...
You are the sequential Group Runner for Observability Release and Continuous Optimization.
Read GLOBAL_AGENT_CONTRACT.md, this RUNNER.md, and all ten specs.
Use one clean branch/worktree dedicated to G20.
