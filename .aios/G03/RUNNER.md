# G03 Group Runner — Provider Pricing and Cost Accounting
...
1. `SPEC-021` — Versioned provider pricing registry
2. `SPEC-022` — Tokenizer abstraction and token estimation
3. `SPEC-023` — Cached-input pricing support
4. `SPEC-024` — Reasoning and tool-call cost accounting
5. `SPEC-025` — Pre-call normal cost estimator
6. `SPEC-026` — Pre-call worst-case cost estimator
7. `SPEC-027` — Actual usage reconciliation
8. `SPEC-028` — Cost event ledger
9. `SPEC-029` — Cost attribution dimensions
10. `SPEC-030` — Pricing freshness and provider-doc verification job
...
- create `artifacts/G03/GROUP_CERTIFICATION.md`
...
You are the sequential Group Runner for Provider Pricing and Cost Accounting.
Read GLOBAL_AGENT_CONTRACT.md, this RUNNER.md, and all ten specs.
Use one clean branch/worktree dedicated to G03.
