...
> Waves are derived by topological longest-path layering of the dependency
> graph in `manifest.json`: every prerequisite group is guaranteed to sit in a
> strictly earlier wave than any group that depends on it. Regenerate with the
> layering tool if `deps` change.
...
After Wave 9: run a dedicated integration session and merge only certified groups.

## Wave 10

- `RUN GROUP G20` — Observability Release and Continuous Optimization — prerequisites: G07, G14, G19

After the final wave: run a dedicated integration session and merge only certified groups.
