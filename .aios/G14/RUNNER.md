# G14 Group Runner — Durable Workflow Runtime
...
1. `SPEC-131` — Workflow template registry
2. `SPEC-132` — Workflow versioning
3. `SPEC-133` — Durable workflow state
4. `SPEC-134` — Step leases and heartbeats
5. `SPEC-135` — Retry classification
6. `SPEC-136` — Idempotency keys
7. `SPEC-137` — Unknown-outcome reconciliation
8. `SPEC-138` — Compensation and saga actions
9. `SPEC-139` — Dead-letter and manual recovery
10. `SPEC-140` — Workflow durability chaos tests
...
- create `artifacts/G14/GROUP_CERTIFICATION.md`

## Required group certification

```text
Group: G14
Specs: SPEC-131..SPEC-140
...
You are the sequential Group Runner for Durable Workflow Runtime.
Read GLOBAL_AGENT_CONTRACT.md, this RUNNER.md, and all ten specs.
Use one clean branch/worktree dedicated to G14.
